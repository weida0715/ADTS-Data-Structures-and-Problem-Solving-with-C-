...
void updateFinancialAid(int numRecords, StudentAidRecord StudentAidRecord[],
                        double percent);
...
/*
    ...
    Precondition: numRecords > 0 and percent > 0 is expressed
        as a decimal
    Postcondition: Each record in finAidArray has been modified by
        increasing the amount by each financial aid award in each
        record by the specified percentage.f
*/
{
